defmodule ASN.Table.IPtoAS do
  ...
  def new(mapping),
    do: mapping |> Enum.reduce(new(), fn {key, value}, acc -> insert(acc, {key, value}) end)
...
defmodule ASN.Table.AStoASN do
  ...
  def insert(table, {key, value}) when is_binary(key),
    do: insert(table, {key |> String.to_integer(), value})

  def insert(table, {key, value}) when is_integer(key),
    do: %AStoASN{table: table.table |> Map.put(key, value)}
